...
use quote::ToTokens;
use syn::{Ident, TraitItemType, TypePath, ImplItemType, parse};
...
#[proc_macro]
pub fn impl_snake_to_camel(input: TokenStream) -> TokenStream {
    let mut item = parse::<ImplItemType>(input).unwrap();
